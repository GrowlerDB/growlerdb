...
- **Fleet-first** default view (all indexes): the aggregate surfaces the outlier rather than making you
  hunt for it. The index-keyed "collapse" cards — query rate, error rate, ingest lag, ingest
  throughput, source rows, small-file size — carry a **"worst / top: `index`"** annotation, and a
  card's expand modal shows a
...
- A persistent **Alerts** strip (critical/warning severity rows, evaluated server-side) above
  **sub-tabs** that group the signals:
  - **Search** — query rate, error rate, full latency (p50/95/99) split into retrieval vs hydrate latency, hydrate rate, stale/drift, cold-cache hit. The query + hydration SLIs are labelled `{index}` at the source, so these are per-index under the scope selector; the **cold-cache hit** card is the one exception (a process-wide LRU with no `{index}`, so it stays cluster-wide). The headline **Query latency** is the *full* GrowlerDB round-trip (`{index, kind, hydrated}` — lexical/semantic/hybrid, with vs without Iceberg hydration), recorded once per request over REST and gRPC; the **Retrieval latency** and **Hydrate latency** cards beneath it split that into where the time went (search vs Iceberg).
