...
- A persistent **Alerts** strip (critical/warning severity rows, evaluated server-side) above
  **sub-tabs** that group the signals:
  - **Search** — query rate, error rate, latency (p50/95/99), hydrate rate/latency, stale/drift, cold-cache hit. The query + hydration SLIs are labelled `{index}` at the source, so these are per-index under the scope selector; the **cold-cache hit** card is the one exception (a process-wide LRU with no `{index}`, so it stays cluster-wide). Query latency/rate/errors are recorded on both the **lexical** and **semantic** paths; a **hybrid** query records once via its lexical arm (so hybrid latency reflects that arm, not the full fused query).
