...
`an.growlerdb.com` — an Apache reverse proxy on the apex VM that forwards to the homelab Plausible
instance over Tailscale — so ad-blockers (heavy among the launch audience) don't strip the beacon.
...
- **Privacy-first, on our infra.** The analytics data stays on our own homelab instance, consistent with
...
- **Endpoint.** `an.growlerdb.com` (short, neutral subdomain — less blockable than an `analytics.*` host)
  resolves to the apex VM (`34.145.3.247`), which reverse-proxies `/js/script.js` and `/api/event` to the
  homelab Plausible over Tailscale. Only those two paths are public; the Plausible **dashboard stays
  tailnet-only**.
- **Snippet.** Both sites load `https://an.growlerdb.com/js/script.js` with a per-site `data-domain`
  (`growlerdb.com` / `docs.growlerdb.com`) so events bucket per site. Events default to the script's own
  origin, so no `data-api` override is needed. The docs snippet lives in
  `docs/_includes/head_custom.html`; the apex snippet is inline in `www/index.html`.
