...
| **Hydration** — `hydrate` / `load_and_plan` / `resolve_pass1` / `point_read` | native store-less pruned key scan | **Trino** key-predicated point `SELECT`, variant column `CAST(... AS JSON)` (`TrinoHydrator::hydrate`). Trino re-finds by key; rows key-verified. |
...
| **Reconciliation** — `scan_stale_index` / `read_documents_in_partition` / `current_plan` / `read_file_key_rows` | native scan + key scan | **Scoped out (interim)** for variant tables: reconciliation is disabled with a D45-loud degradation flag; the connector's changelog is the source of truth until the native path lands. |
