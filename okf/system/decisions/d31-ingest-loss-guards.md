...
from a post-D30 microk8s validation that found a shard short 6 rows with no error (task-194):
...
1. **Expected-row-count gate (connector).** Before any write for a trigger, sum `added-records`
   ...
   an empty/short window used to jump the in-memory cursor to head and a later batch stamped a later
   checkpoint over the gap.
2. **Node checkpoint-continuity guard.** Each `DocBatch` now carries the `from` checkpoint it
   resumes from. *(As shipped: exact `from == current`. Relaxed by task-196 to **window-covering**,
   ...
   the stage-time check is lock-free and advisory (task-207 closed the stage/commit TOCTOU that let
   two racing writers regress the checkpoint).
...
**Why.** The audit (`backlog/docs/ingest-loss-audit.md`) traced the loss to an under-read window
...
node originally couldn't compare checkpoints at all and continuity was an **exact `from ==
...
**Refinement (task-196): ordered checkpoints + window-covering.** Exact matching turned out to be
...
bug (task-205). Too brittle: the exact lattice depends on re-sent batch ids matching originals,
...
unmatchable `CHECKPOINT_GAP` forever (task-206). The fix is one primitive plus one relaxation:
...
migration clears the misordered pre-task-196 index — safe, because under covering the batch-id
...
the sharded, scheduled reconcile backstop D9 promises** (task-195, shipped shard-scoped). The guards
...
lockstep empty-advance against the in-process shard). The original **task-194 loss signature** — one
...
advancing in the first place. Refined by **task-196** (ordered checkpoints
+ window-covering guard + commit-time re-check, closing task-205/206/207) — the enabling
