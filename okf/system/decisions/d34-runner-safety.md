...
description: On going public, untrusted fork-PR code never runs on the self-hosted runners — an org policy requires approval for all outside collaborators, and pull_request CI runs on disposable GitHub-hosted runners while self-hosted is reserved for push(main) and nightly; CI tokens are least-privilege.
...
runners**. Two independent layers enforce this:
...
2. **Hosted PR CI.** `pull_request` jobs run on **GitHub-hosted** runners
   ...
   work. So even an *approved* fork PR executes on disposable infra, never the self-hosted runners.
...
**Why.** Public + unguarded self-hosted runners = **arbitrary code execution on the self-hosted
runners** from any fork PR — it inverts the private-repo trust
...
the self-hosted runners to attacker-controlled code. GitHub-hosted minutes are **free for public repos**, so
...
branching, at the cost of not using the self-hosted runners for insiders' PRs. The required `ci-gate` status
...
**Status.** Accepted. Approval policy applied org-wide; `ci.yml` and `connector.yml`
carry the per-event runner split; `nightly.yml` stays self-hosted (schedule/dispatch only — no fork
PRs reach it). Precedes going public + Pages.
