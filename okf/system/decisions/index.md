...
* [D36. License: AGPLv3](/system/decisions/d36-license-agplv3.md) - GrowlerDB is licensed AGPL-3.0-only; a commercial license is available for uses that cannot accept AGPL.
* [D37. Extension seams for out-of-tree capabilities](/system/decisions/d37-extension-seams.md) - The core exposes pluggable behaviour as stable public trait seams; out-of-tree implementations attach without forking, and the default build stays 100% AGPL.
* [D38. Scale-limit entitlement (offline license)](/system/decisions/d38-scale-limit-entitlement.md) - The free tier runs up to a fixed node count; more requires an offline-verified Ed25519 license. New nodes are capped; existing nodes are never disrupted.
* [D39. Automatic cold-tiering is node-local, not a scheduled job](/system/decisions/d39-automatic-cold-tiering.md) - Each node parks its own aged windows to cold read-through in-process on a timer (the hot→cold counterpart of pre-warm), because parking needs the window's bytes on the node's own local volume.
* [D40. Unified brand system (Brand v1.0)](/system/decisions/d40-brand-system.md) - One dark-first brand — waterline mark, neutral palette with glacier primary / melt identity / amber warnings, and the Archivo / Instrument Sans / Geist Mono trio — across console, website, docs, and social, replacing the fragmented pre-GA surfaces; the "GA" mock copy is superseded by the current Beta maturity.
