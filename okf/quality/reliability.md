...
- **Chaos drills** — repeatable fault-injection: process crash (self-restart + `/readyz` recovery +
  ...
  a `cargo test` drill (`chaos_failover`) stands up a control plane at `R=2` + two pool nodes serving
  **two** parked windows read-through from a shared object store (the `GROWLERDB_OBJECT_STORE_FS`
  local-fs mode, so no MinIO), **kills the primary node under sustained query against a gateway built
  before the kill** (established channels, not a fresh dial) and asserts a **bounded failover gap**:
  after a short grace every query answers both windows via the replica on the multi-window scatter
  path — where `partial` is structurally reachable — with no `partial`. Compose drills run against a
  live stack; a Kubernetes chaos harness runs on the cluster.
