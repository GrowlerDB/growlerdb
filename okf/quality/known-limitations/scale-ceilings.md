...
[layered locator](/system/decisions/d30-layered-locator.md) claim holds) but the O(rows) location array is the residual and it is always hot.
...
3. Then the structural bets in priority order: bounded compaction (done), search
   ...
   remains (ceiling #3, the `location.arr` hot floor) is a bounded residual, not a blocker.
