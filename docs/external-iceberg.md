...
The [getting-started demo](getting-started) runs GrowlerDB against a bundled MinIO + Polaris + a
seeded table. This is the **day-2 step**: run GrowlerDB with Docker Compose against **your own
...
They authenticate to S3 **differently** (the engine uses `GROWLERDB_S3_ACCESS_KEY`/`SECRET_KEY`; the
connector uses the AWS SDK's `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`) — set both.

## Before you start

You need an **Iceberg table already registered in a REST catalog** (Apache Polaris, Nessie, or any
Iceberg REST catalog), stored on **S3-compatible object storage**, both reachable from where you run
Compose. See the [limitations](#limitations) below — notably the catalog must speak the **Iceberg REST
protocol** (AWS Glue and Hadoop catalogs are not supported by the engine).
...
MinIO/Polaris/seed — it runs only GrowlerDB (control plane + node + gateway) off the published image,
with every connection setting coming from a `.env` file.
...
Key `--conf` notes: `type=rest` selects the REST catalog; `cache-enabled=false` is required for
streaming so each trigger sees new snapshots; `io-impl=…S3FileIO` + the `s3.*` settings point Spark at
your bucket. For **AWS S3** use `s3.path-style-access=false` (virtual-hosted); for **MinIO** use
...
These are real constraints of the current engine — plan around them:
...
- **Static S3 keys only.** The engine authenticates to S3 with a static access key + secret key. There
  is no IAM instance-role / STS / assume-role support on the engine side — supply
  `GROWLERDB_S3_ACCESS_KEY`/`SECRET_KEY`.
- **Path-style S3 access is forced on** by the engine. This is required for MinIO and still works with
...
- **Rotate the secrets.** `GROWLERDB_SERVICE_TOKEN` (mesh auth) and `GROWLERDB_AUTH_SECRET` (gateway
  login) default to placeholders in the template — set real values.
...
- **Catalog `401`/`403`** — check `GROWLERDB_CATALOG_CREDENTIAL` (`id:secret`) and, for Polaris,
  ...
  must carry them.
- **S3 access denied / no such bucket** — the engine and the connector authenticate separately; verify
...
Compose is for local runs and experiments. For a real deployment, use the **Helm chart**, which is
built for external catalogs + object stores and takes credentials from a Kubernetes `Secret`. See the
