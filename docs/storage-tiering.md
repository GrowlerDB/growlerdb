...
- Cold reads are slower, but never unavailable. A cold window is queried in place (read-through from
  ...
  range-read latency against an NVMe hit, warming into the cache as it goes. Window pruning means most
  queries never touch cold at all, and the cold-cache hit-rate is exposed as an SLI.
...
land in the *current* window, so old windows stay immutable and parkable. To keep event-time queries
...
fast. The cost is that a wide backfill broadens a window's event-time range and reduces pruning for
that window (it gets scanned, never mis-answered).
