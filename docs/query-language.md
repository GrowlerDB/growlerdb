...
GrowlerDB has a canonical structured query AST, and the string forms below parse into it. Most
...
The string parses into these nodes, which the structured API and the
[OpenSearch adapter](opensearch-adapter) target directly:
...
- `sort`: sort keys (a composite-key tiebreaker is applied implicitly); empty means by relevance,
  `_score` descending. Each key is a numeric/date/KEYWORD fast field, or the reserved `_score`
  for relevance, either alone (`[{ "field": "_score", "desc": true }]`) or as a tiebreaker among fields
...
- `search_after`: opaque keyset cursor from a prior response's `next_cursor` (stable deep
  paging; requires a sort over fast fields, and a `_score` sort is rejected here).
...
Hits are document coordinates (the composite key) plus a BM25 score, not the documents themselves.
Fetch the authoritative rows from Iceberg by key with [`/v1/keys:get`](rest-api#post-v1keysget).
`total` is a true cross-shard match count, and a `partial` flag is set if any shard was down. When the
request opts into `highlight`, each hit also carries a `highlight` object (field → fragments → XSS-safe
