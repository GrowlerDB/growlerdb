...
mod tests {
    ...
    async fn config_dto_has_no_secret_field() {
        ...
        // No key material, and no key-shaped field name, appears in the serialized response.
        assert!(
            !text.contains("sk-must-not-leak"),
            "config leaked a key: {text}"
        );
        let body: JsonValue = serde_json::from_str(&text).unwrap();
        let obj = body.as_object().unwrap();
        for k in obj.keys() {
            let lk = k.to_lowercase();
            assert!(
                !lk.contains("key") && !lk.contains("secret") && !lk.contains("token"),
