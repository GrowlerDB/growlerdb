//! Small shared helpers for the gRPC service impls: map errors to a tonic [`Status`] and run
...
/// The "index not served by this node" guard shared by the single-index Node RPCs: an empty
/// `requested` name means "the served index"; any other name is `NotFound`.
