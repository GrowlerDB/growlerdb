...
fn spawn_locator_remap(
    ...
    tokio::spawn(async move {
        ...
        loop {
            tick.tick().await;
            // Pin the current shard(s) per tick (a reindex swap is respected next tick).
            let shards: Vec<std::sync::Arc<growlerdb_index::Shard>> =
                handles.iter().map(|h| h.current()).collect();
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let opened = tokio::task::spawn_blocking(move || {
                store2.open_shard(&ShardId::window(&index2, window), &resolved2)
            })
            .await;
            match opened {
                Ok(Ok(shard)) => {
...
async fn gateway(cfg: GatewayConfig<'_>) -> anyhow::Result<()> {
    use std::sync::Arc;
    use tonic::transport::Server;
    ...
    let gw = if let Some(issuer) = oidc_issuer {
