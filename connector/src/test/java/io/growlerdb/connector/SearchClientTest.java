...
class SearchClientTest {
  ...
  void searchForwardsTheQueryAndReturnsHits() throws Exception {
        ...
        ServerBuilder.forPort(0)
            .addService(
                new SearchGrpc.SearchImplBase() {
                  @Override
                  public void search(
                      SearchRequest request, StreamObserver<SearchResponse> obs) {
                    seen.set(request);
                    SearchHit hit =
                        SearchHit.newBuilder()
                            ...
                            .setScore(1.5)
                            .build();
                    obs.onNext(SearchResponse.newBuilder().addHits(hit).setTotal(1).build());
                    obs.onCompleted();
                  }
                })
            .build()
    ...
    try (SearchClient client = new SearchClient("127.0.0.1", server.getPort())) {
      ...
      assertEquals(1.5, hits.get(0).getScore());
    } finally {
